                        ...
                        <!-- form start -->
                            ...
                            <div v-if="taskForEdit.editing == false" class="box-body">
                                ...
                                <div class="form-group">
                                    <label>User id</label>
                                    <td type="text" class="form-control" v-html="taskForEdit.task.user_id"></td>
                                </div>
                                <div class="form-group">
        ...
        <div v-cloak>
            ...
            <table class="table table-bordered table-hover" dusk="tasks">
                ...
                <tr v-for="(task, index) in filteredTasks" dusk="tasks" id="tasks">
                    ...
                    <td v-if="userForEdit.index == index && userForEdit.field == 'user'">
                        <select @change="takeUserForEdit(task, index, 'user')" v-model="task.user_id">
                            <option v-for="user in users" :value="user.id">{{user.name}}</option>
                        </select>
                    </td>
                    <td v-else @click="takeUserForEdit(task, index, 'user')">{{task.user_id}}</td>
                    ...
                    <td class="action">
                        <div class="align-center btn-group">
                            <button v-if="index == taskForEdit.index && taskForEdit.editing == true || (userForEdit.editing == true && userForEdit.index == index)" type="button" @click="saveTask" class="btn btn-default" alt="Save Task">
  ...
  export default {
    ...
    methods: {
      ...
      takeTaskForEdit(task, index, field){
        ...
        this.userForEdit.task = task;
      },

      takeUserForEdit(task, index, field){

        var user = this.users[task.user_id];
        console.log(this.users[task.user_id-1].name);
        this.userForEdit.task = task;
        this.userForEdit.index = index;
        this.userForEdit.editing = true;
        this.userForEdit.field = field;

        this.taskForEdit.task = task;
        console.log(this.taskForEdit.task);
      ...
      saveTask() {
        ...
        var name = '';
        var user_id = this.userForEdit.task.user_id;
        console.log(user_id);

        (this.editor == '') ? name = this.taskForEdit.task.name : (this.editor == 'medium-editor') ? name = this.modifiedName : name = this.taskForEdit.task.name;


        (this.editor == 'medium-editor') ? description = this.modifiedDescription : description = this.taskForEdit.task.description;

        axios.put(url, {name: name, description: description, user_id: user_id }).then((response) =>  {

          this.tasks[pos].name = name;
          this.tasks[pos].description = description;
          this.tasks[pos].user_id = user_id;
          ...
          this.userForEdit.field = '';
          this.userForEdit.user_id = -1;
          this.userForEdit.index = -1;
          this.userForEdit.editing = false;
